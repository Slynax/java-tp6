...
public class EntityManagerImpl implements EntityManager {
    ...
    public EntityManagerImpl() {
        try {
            ...
            this.connection.createStatement().executeUpdate(
                    "CREATE TABLE IF NOT EXISTS Club (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1, INCREMENT BY 1) PRIMARY KEY, version INT, fabricant VARCHAR(255), poids FLOAT)");
    ...
    public void persist(Object entity) {
        if (entity instanceof Club) {
            ...
            String sql = "INSERT INTO Club (version, fabricant, poids) VALUES (?, ?, ?)";

            try (PreparedStatement statement = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                Object versionValue = getFieldValue(entity, "version");
                if (versionValue instanceof Integer) {
                    statement.setInt(1, (Integer) versionValue);
                } else {
                    throw new IllegalArgumentException("La version n'est pas un entier");
                }
                ...
                try (ResultSet generatedKeys = statement.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        setFieldValue(club, "id", generatedKeys.getLong(1));
                    }
                }
                connection.commit();
            } catch (SQLException e) {
